...
class CodeforcesRandomDialog(private val project: Project) : DialogWrapper(project) {
    ...
    private val solvedFilterCombo = ComboBox(arrayOf(
        ...
        isEnabled = loggedIn
        if (!loggedIn) {
            toolTipText = I18n.t("Codeforces 로그인이 필요합니다", "Codeforces login required")
            val combo = this
            addMouseListener(object : MouseAdapter() {
                override fun mousePressed(e: MouseEvent) {
                    JBPopupFactory.getInstance()
                        .createHtmlTextBalloonBuilder(
                            I18n.t("풀이 필터를 사용하려면<br>Codeforces에 로그인하세요", "Login to Codeforces<br>to use solved filter"),
