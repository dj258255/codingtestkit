...
class SwexpertRandomDialog(private val project: Project) : DialogWrapper(project) {
    ...
    private val solvedFilterCombo = ComboBox(arrayOf(
        I18n.t("전체", "All"),
        I18n.t("내가 푼 문제 제외", "Exclude Solved"),
        I18n.t("내가 푼 문제만", "Solved Only")
    )).apply {
    ...
    private val pickButton = JButton(I18n.t("뽑기", "Pick"))
    ...
    init {
        title = I18n.t("SWEA 랜덤 문제 뽑기", "SWEA Random Problem Picker")
    ...
    override fun doOKAction() {
        if (!doubleClicked) {
            selectedProblemIds = checkedRows.filter { it in results.indices }.map { results[it].contestProbId }
        }
        super.doOKAction()
    }
    ...
    override fun createCenterPanel(): JComponent {
        ...
        // Row 2: 언어 + 풀이 필터
        val row2 = JPanel(FlowLayout(FlowLayout.LEFT, JBUI.scale(6), JBUI.scale(2))).apply {
            alignmentX = Component.LEFT_ALIGNMENT; maximumSize = Dimension(dialogWidth, JBUI.scale(40))
        }
        row2.add(createLabel(I18n.t("언어", "Language")))
        row2.add(languageCombo)
        row2.add(Box.createHorizontalStrut(JBUI.scale(12)))
        row2.add(createLabel(I18n.t("풀이 상태", "Solve Status")))
        row2.add(solvedFilterCombo)
        topPanel.add(row2)
        ...
        row3.add(createLabel(I18n.t("개수", "Count")))
        row3.add(countField)
        row3.add(Box.createHorizontalStrut(JBUI.scale(12)))
        row3.add(createLabel(I18n.t("최소 참여자", "Min Participants")))
        row3.add(minParticipantsField)
        row3.add(pickButton)
        topPanel.add(row3)
        ...
        resultTable.columnModel.getColumn(2).preferredWidth = JBUI.scale(270)
        resultTable.columnModel.getColumn(3).preferredWidth = JBUI.scale(45)
        resultTable.columnModel.getColumn(4).preferredWidth = JBUI.scale(70)
        resultTable.columnModel.getColumn(5).preferredWidth = JBUI.scale(55)
        ...
        pickButton.addActionListener { doRandom() }
        resultTable.addMouseListener(object : MouseAdapter() {
            override fun mouseClicked(e: MouseEvent) {
                if (e.clickCount == 2 && resultTable.selectedRow >= 0) {
                    selectedProblemIds = listOf(results[resultTable.selectedRow].contestProbId)
                    doubleClicked = true
                    doOKAction()
                }
            }
    ...
    private fun doRandom() {
        ...
        val solvedFilter = solvedFilterCombo.selectedIndex
        ...
        Thread {
            try {
                results = SwexpertApi.randomProblems(
                    levels = levels,
                    language = language,
                    count = count,
